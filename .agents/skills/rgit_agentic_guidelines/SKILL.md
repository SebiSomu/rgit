...
- **Helper Functions**: Use the helper functions in `helpers.rs` for reading and writing compressed Git objects, walking the commit graph, and matching file status. When thinking of adding a new feature soon and a new helper function is needed for it, always implement the helper function in helpers.rs before implementing the feature itself. All helper functions should have a comment written above them explaining what they do and for which feature they are needed to be used for.
